...
const Skills = () => {
  ...
  const secondRow = mockSkills.slice(
    (mockSkills.length - 1) / 2,
    mockSkills.length
  );
  ...
  return (
    <div className='h-full relative after:absolute after:w-11 after:h-full  after:bg-gradient-to-r after:from-neutral-900  after:to-transparent  after:top-0 before:absolute before:w-11 before:h-full before:z-[1] before:bg-gradient-to-l before:from-neutral-900 before:to-transparent before:right-0 '>
...
const RenderChip = ({ title, icon, index }: SkillsProps) => {
  return (
    <span className='cursor-pointer flex w-fit justify-center items-center px-4 py-2 gap-1 rounded-full border border-neutral-400 bg-neutral-900 bg-opacity-90'>
      <i className={`${icon}`}></i>
      {title ? <span className='text-sm text-nowrap'>{title}</span> : null}
    </span>
  );
};
